...
export default function PublicProfilePage() {
    ...
    const fetchProfile = async () => {
        try {
            ...
            // Fetch profile by username (service handles visibility check)
            const profileData = await userProfileService.getPublicProfile(username);

            console.log('Profile data received:', profileData ? 'Found' : 'Not found');

            if (!profileData) {
                setError(`Profile "${username}" not found or is private`);
                return;
            }
            ...
            console.log('Stats and artworks loaded:', { stats: statsData, artworksCount: artworksData.length });
    ...
    const handleShare = async () => {
        // Always use username for share links
        if (!profile?.username) {
            toast({
                title: "Cannot Share",
